...
| Switchyard observation | The optional Switchyard adapter sends bounded content-free observations and records agreement, latency, and error aggregates. It is observe-only: it cannot change the plan, target, authority, dispatch, fallback, kill state, or native result. Remote use requires acknowledged HTTPS. The adapter targets the experimental NVIDIA NeMo Relay 0.6.0 Switchyard decision API, which NVIDIA removes in NeMo Relay 0.8, so treat it as a pilot against a moving external contract. |
